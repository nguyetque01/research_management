...
function ArticleForm({
  ...
  useEffect(() => {
    // Tạo một hàm để kiểm soát việc load dữ liệu từ editingArticle
    const loadFormData = async () => {
      if (editingArticle) {
        // Nếu đang chỉnh sửa bài báo, cập nhật formData từ editingArticle
        setFormData({ ...editingArticle });
      } else {
        // Nếu đang thêm bài báo mới, cập nhật formData từ newArticle
        setFormData({ ...newArticle });
      }

      // Đã load xong dữ liệu từ editingArticle
    ...
    <div style={customFormContainerStyle}>
      <div style={headerStyle}>
        <Typography variant="h6">
          {editingArticle ? "Chỉnh sửa bài báo" : "Thêm bài báo"}
        </Typography>
      </div>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6} sx={{ mt: 1 }}>
                <TextField
                  fullWidth
                  name="published_type"
                  label="Loại hình xuất bản"
                  value={formData.published_type}
                  onChange={handleChange}
                />
              </Grid>
              <Grid item xs={12}>
